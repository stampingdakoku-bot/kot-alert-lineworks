...
## 管理画面
- URL: https://133-125-93-39.sslip.io/ （HTTPは自動でHTTPSへリダイレクト）
- 旧URL: http://133.125.93.39/ （現在はHTTPS版へ301リダイレクト）
- 勤怠ボード(NeeSa): https://133-125-93-39.sslip.io/board
- パスコード認証は廃止（ログアウトボタンも非表示）
- 機能:
...
## 注意事項
- HTTPS有効（sslip.io無料サブドメイン + Let's Encrypt、certbot自動更新）。HTTPは全てHTTPSへ301リダイレクト
...
- マッピング漏れ5名を督促対象として登録済み（矢幡4011・金広1021・河野1022・杉村4012・日高4013）。味志祥太朗4014は退職予定のため未登録。LW IDはUUID形式またはログインID形式(av.xxxxx@... 等)どちらでも送信可（佐久間・吉武・坂本で確認済み、2026-06-08）。当時UUID必須と誤認しUUIDで登録したが実害なし
...
- LW ID修正: 水津・西口のlw_account_idをログインID形式からUUID形式に更新(/users/{loginId} APIで取得)。水津=ae907a50-1ec2-4af4-1ab5-0442e53994f9、西口=a84486af-626d-4593-1fb8-042503356746。※後日ログインID形式でも送信可と判明(2026-06-08)。UUID形式のままでも問題なし
...
## HTTPS化（2026-06-30 完了）
- 無料サブドメイン sslip.io（`133-125-93-39.sslip.io` → 133.125.93.39、登録不要）
- Let's Encrypt証明書をcertbotで発行（python3-certbot-nginx、有効期限90日・certbot.timerで自動更新、dry-run成功確認済み）
- Nginx: :443 SSL追加、:80は全て `https://133-125-93-39.sslip.io` へ301リダイレクト（/.well-known/acme-challenge/ はHTTPで通す）
- 設定ファイル: /etc/nginx/sites-enabled/kot-alert（バックアップは /home/ubuntu/nginx-backups/ ※sites-enabled内に置くとserver_name重複で誤読込するので注意）
- 注意: nginxは `include /etc/nginx/sites-enabled/*` なので、このディレクトリにバックアップ等を置かないこと

## 既知の課題
- トレコレ査定ツール（別VPS 49.212.220.117:5000）のHTTPS化は未対応（同様にsslip.io+Let's Encryptで可能）
- マネフォビジネス給与API連携検討中
- KoT overtimes API未検証

## 2026-06-08 更新
- commit 8983651: clock_error_reminder の最終段(remind_count>=3)テンプレを「最終警告」文面に差し替え。坂本(9998)テスト送信で実機表示確認済み
- commit 29987fc: send_daily_report(13:10管理グループまとめ)冒頭に2種リマインド追加
  - 14日 → 締め版「⚠️ 明日15日は勤怠の最終締め日です」（土日祝でも必ず送る）
  - 月曜/水曜 かつ当日・翌日とも祝日でない → 定例版「📋 明日は総務による勤怠のチェック日です」
  - 優先順位: 14日→月水→なし（両方は出さない）。判定は jpholiday.is_holiday()
  - 既存まとめ本体・送信処理・宛先・二重送信ガード無変更。1日1通のまま
  - 本番デビュー: 定例版=2026-06-10(水)、締め版=毎月14日
- jpholiday 1.0.3 導入、requirements.txt追記（祝日判定の基盤）
- LINE WORKS Bot送信: ログインID形式でもUUID形式でも送信成功(200/201)と判明。従来の「UUID必須」は誤り

## 2026-08-03 保守引き継ぎ完了
- 松田さんへ引き継ぎ完了。GitHub/VPS SSH/Supabase/管理Google全アクセス確認済み
